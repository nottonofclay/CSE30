...
d2 = [choice(list_) for i in range(50)]  # random list of size 50
d3 = [choice(list_) for i in range(100)]  # random list of size 100
d4 = [choice(list_) for i in range(200)]  # random list of size 200
d5 = [choice(list_) for i in range(500)]  # random list of size 500

# you need to add more lists of different sizes: d3, d4, d5, and d6
data = [d1, d2, d3, d4, d5]  # your input
times = []       # times required to sort input

for i in data:
    t1 = Timer(f"bubbleSort({i})", "from __main__ import bubbleSort")
    print("bubblesort ",t1.timeit(number=10000), "milliseconds") # for debugging
    times.append('hi') # record the time required to sort the input

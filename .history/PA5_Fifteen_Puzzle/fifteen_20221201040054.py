...
class Fifteen:
    ...
    def draw(self):
        board_vals = self.tiles.get_verticies_values()
        print('+----+----+----+----+')
        print('| {{width}} | {{width}} | {{width}} | {{width}} |'.format(board_vals[0], board_vals[1],  board_vals[2],  board_vals[3], fill=' ',width=10))
        print('+----+----+----+----+')
        print('| {{width}} | {{width}} | {{width}} | {{width}} |'.format(board_vals[4], board_vals[5],  board_vals[6],  board_vals[7], width=2))
        print('+----+----+----+----+')
        print('| {{width}} | {{width}} | {{width}} | {{width}} |'.format(board_vals[8], board_vals[9],  board_vals[10], board_vals[11], width=2))
        print('+----+----+----+----+')
        print('| {{width}} | {{width}} | {{width}} | {{width}} |'.format(board_vals[12],board_vals[13], board_vals[14], board_vals[15], width=2))
        print('+----+----+----+----+')
    ...
    def __str__(self):
        ...
        for i in range(2, len(self.tiles.get_verticies_values())):
            ...
            if (current.value in previous.get_connections()):
                if (current.value < 10) and (previous.value < 10):

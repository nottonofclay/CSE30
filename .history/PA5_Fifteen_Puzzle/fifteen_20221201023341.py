...
class Fifteen:
    ...
    def update(self, move):
        if (self.tiles.is_valid_move(move, 16)):
            self.transpose()

    def transpose(self, i, j):
        if (self.tiles.is_valid_move(i,j)):
            self.tiles.get_vertex(i).value, self.tiles.get_vertex(j).value = self.tiles.get_vertex(j), self.tiles.get_vertex(i)
...
if __name__ == '__main__':
    ...
    for i in game.tiles:
        print(i)
    print(game.is_valid_move(12, 16))

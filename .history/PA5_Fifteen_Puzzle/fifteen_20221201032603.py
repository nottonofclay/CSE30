...
class Fifteen:
    ...
    def __str__(self):
        ...
        for i in range(2, len(self.tiles.get_verticies_values())+1):
            first = self.tiles.get_vertex(i)
            last = self.tiles.get_vertex(i-1)
            if (first.value in last.get_connections()):
                if (first.value < 10):
                    output += ' ' + str(first.value) + ' '
                else:
                    output += str(first.value) + ' '
            else:
                if (first.value == ' ') and (self.tiles.get_vertex(i+1).value > 9):
                    output += first.value + '  '
                elif (first.value == ' '):
                    output += first.value + ' '
                elif (first.value < 10) and (self.tiles.get_vertex(i+1).value > 9):
                    output += '\n' + str(first.value) + '  '
                else:
                    output += '\n' + str(first.value) + ' '
        return output

...
class Fifteen:
    ...
    def __str__(self):
        output = self.tiles.get_vertex(1).value
        for i in range(2, len(self.tiles.get_verticies_values())+1):
            if (self.tiles.get_vertex(i) in self.tiles.get_vertex(i-1).get_connections()):
                output += self.tiles.get_vertex(i).value + ' '
            else:
                output += '\n' + self.tiles.get_vertex(i).value + ' '
